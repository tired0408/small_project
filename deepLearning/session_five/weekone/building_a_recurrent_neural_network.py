...
Waa = np.random.randn(5, 5)
Wya = np.random.randn(2, 5)
b = np.random.randn(5, 1)  # 这处应该是ba可能是课程的错误
by = np.random.randn(2, 1)
...
def rnn_backward(da, caches):
    ...
    for t in reversed(range(T_x)):
        ...
        gradients = rnn_cell_backward(da[:, :, t] + da_prevt, caches[t]) # TODO 为啥要加上da_prevt,每步增加一定的偏移量？
...
print("gradients[\"dWax\"][3][1] = ", gradients["dWax"][3][1])
print("gradients[\"dWax\"].shape = ", gradients["dWax"].shape)
print("gradients[\"dWaa\"].shape = ", gradients["dWaa"][1][2])
print("gradients[\"dWaa\"].shape = ", gradients["dWaa"].shape)
print("gradients[\"dba\"][4] = ", gradients["dba"][4])
print("gradients[\"dba\"].shape = ", gradients["dba"].shape)
print("-"*100)
def lstm_cell_backward(da_next, dc_next, cache):
    ...
    # TODO 错题记录，错误的写法，正确答案好像跟前面的公式不太一样
    ...
    gradients = {"dxt": dxt, "da_prev": da_prev, "dc_prev": dc_prev, "dWf": dWf, "dbf": dbf, "dWi": dWi, "dbi": dbi, "dWc": dWc,
                 "dbc": dbc, "dWo": dWo, "dbo": dbo}
    return gradients
np.random.seed(1)   # TODO 不是很熟悉，有待进一步研究
xt = np.random.randn(3, 10)
a_prev = np.random.randn(5, 10)
c_prev = np.random.randn(5, 10)
Wf = np.random.randn(5, 5+3)
...
def lstm_backward(da, caches):
    ...
    ### END CODE HERE ###
    # Store the gradients in a python dictionary
    gradients = {"dx": dx, "da0": da0, "dWf": dWf, "dbf": dbf, "dWi": dWi, "dbi": dbi, "dWc": dWc, "dbc": dbc, "dWo": dWo,
                 "dbo": dbo}
    return gradients
np.random.seed(1)
x = np.random.randn(3, 10, 7)
